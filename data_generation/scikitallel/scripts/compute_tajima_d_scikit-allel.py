...
sci_pi_file = "data/tajima_d_out/allel_" + re.sub('.*/|_invar.*', '', vcf_path) + re.sub('.vcf', '', NUM_ID)  + "_tajima_d.txt"
outfile = open(sci_pi_file, 'w')
outfile.write("filename" + "\t" + "chromosome" + "\t" + "window_pos_1" + "\t" + "window_pos_2" + "\t" + "sk_allel_tajima_d" + "\t" + "sk_allel_no_sites" + "\n")

# compute pi (for the whole region)
hap = gt_region.to_haplotypes() 
ac = hap.count_alleles()
d_est = allel.tajima_d(ac) 

# write to file
outfile.write(re.sub('.*/|.vcf.*', '', vcf_path) + "\t" + str(chromosome) + "\t" + str(window_start) + "\t" + str(window_end) + "\t" + str(d_est) + "\t" + str(len(gt_region)) + "\n")
outfile.close()

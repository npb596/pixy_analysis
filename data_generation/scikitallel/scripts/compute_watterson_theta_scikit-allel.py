...
NUM_ID=re.sub('.*invar.', '_', vcf_path)
# create pi name via the prefix
sci_pi_file = "data/watterson_theta_out/allel_" + re.sub('.*/|_invar.*', '', vcf_path) + re.sub('.vcf', '', NUM_ID)  + "_watterson_theta.txt"
outfile = open(sci_pi_file, 'w')
outfile.write("filename" + "\t" + "chromosome" + "\t" + "window_pos_1" + "\t" + "window_pos_2" + "\t" + "sk_allel_avg_watterson_theta" + "\t" + "sk_allel_no_sites" + "\n")

# compute pi (for the whole region)
hap = gt_region.to_haplotypes() 
ac = hap.count_alleles()
theta_est = allel.watterson_theta(window_range, ac) 

# write to file
outfile.write(re.sub('.*/|.vcf.*', '', vcf_path) + "\t" + str(chromosome) + "\t" + str(window_start) + "\t" + str(window_end) + "\t" + str(theta_est) + "\t" + str(len(gt_region)) + "\n")
outfile.close()
